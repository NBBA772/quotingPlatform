      ...
      >
        ...
        >
          <div class="max-w-6xl mx-auto">
            <h2 class="text-4xl md:text-6xl text-4xl font-bold">{{ slide.title }}</h2>
            <p class="text-lg mt-2">{{ slide.description }}</p>

            <!-- Render custom content if present -->
            <component v-if="slide.content" :is="slide.content" />

            <div class="mt-10 flex items-center justify-center gap-x-6">
              <NuxtLink
                v-if="slide.buttonText"
                :to="slide.link"
                class="px-4 py-2 bg-blue-500 dark:bg-[#046937] rounded mt-4 text-white"
              >
                {{ slide.buttonText }}
              </NuxtLink>

              <NuxtLink
                v-if="slide.buttonText2"
                :to="slide.link2"
                class="px-4 py-2 border  rounded mt-4 text-white"
              >
                {{ slide.buttonText2 }}
              </NuxtLink>
            </div>
...
export default {
  ...
  setup(props) {
    ...
    const reloadSlider = () => {
      ...
      refreshInterval = setInterval(nextSlide, 10000);
    };
