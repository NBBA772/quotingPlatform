        ...
        <table class="w-full border-collapse border border-gray-300 dark:border-gray-600">
          ...
          <tbody>
            <tr>
              ...
              <td class="border p-2">
                Sends an invite email. If <code>mock</code> is true, it renders a preview instead of
                sending.
              </td>
            </tr>
          </tbody>
...
// Replace `company` info as needed
const company = {
...
const sendInvite = async () => {
  ...
  try {
    if (props.mock) {
      ...
      const response = await $fetch("/api/company/send-invite", {
        method: "POST",
