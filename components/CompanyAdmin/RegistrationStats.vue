<template>
  <div
    class="p-4 border border-gray-200 dark:border-gray-700 shadow rounded-lg bg-white dark:bg-[#142610]"
  >
    <!-- Skeleton Loader -->
 <CompanyAdminRegistrationStatsSkeletonLoader v-if="loading"  />
    ...
    <!-- Actual Content -->
    <div v-else>
      <h2 class="text-lg font-bold mb-2">Registration Completion Rate</h2>

      <!-- Info -->
      <p class="text-sm text-white dark:text-gray-300 mb-2">
        {{ registeredCount }} of {{ invitedCount }} employees registered
      </p>

      <!-- Progress Bar -->
      <div
        class="w-full bg-gray-200 dark:bg-[#3a4934] rounded-full h-3 overflow-hidden"
      >
        <div
      ...
      <div v-if="props.mock">
          ...
          >
            ...
            <tbody>
              <tr>
                ...
                <td class="border p-2">
                  If true, uses fake registration stats and displays props &
                  functions tables.
                </td>
              </tr>
            </tbody>
        ...
        <div class="mt-8">
          ...
          >
            ...
            <tbody>
              <tr>
                <td class="border p-2">fetchRegistrationData</td>
                <td class="border p-2">None</td>
                <td class="border p-2">Promise&lt;void&gt;</td>
                <td class="border p-2">
                  Fetches registration stats. If
                  <code>mock</code> is true, loads fake data instead of API call.
                </td>
              </tr>
            </tbody>
          </table>
...
// Props
const props = withDefaults(defineProps<{ mock?: boolean }>(), {
  mock: false,
});

const loading = ref(true); // <-- loading ref
...
async function fetchRegistrationData() {
  if (props.mock) {
    invitedCount.value = 25;
    registeredCount.value = 17;
    loading.value = false; // done loading
    return;
  }

  try {
    const data = await $fetch("/api/company/registration-stats");
