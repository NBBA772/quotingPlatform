  ...
  <div class="p-4 my-4 shadow rounded-lg bg-white dark:bg-[#3a4934]">
    ...
    <form v-if="open" class="mt-4 space-y-4" @submit.prevent="submit">
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400">
        The admin logs in with their email and this password.
      </p>
...
// Default the username to the email until the admin types their own.
...
async function submit() {
  ...
  try {
    ...
    error.value = err?.data?.statusMessage || err?.message || 'Failed to create admin login'
  } finally {
    saving.value = false
  }
}
