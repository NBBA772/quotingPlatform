        ...
        <tr v-for="trail in pagedTrails" :key="trail.id" class="hover:bg-gray-50 dark:hover:bg-[#2d3a2a]">
          ...
          <td class="p-2">
            <div class="flex items-center gap-2">
              <button class="text-gray-500 hover:text-red-600" @click="confirmRemove(trail.id)" title="Move to processed (soft delete)">
...
// Keep the page in range as rows are removed.
...
onMounted(async () => {
  try {
    const res = await $fetch("/api/admin/audit-trails/active"); // only active (deletedAt = null)
...
const confirmRemove = async (id: number) => {
  ...
  try {
    await $fetch(`/api/admin/audit-trails/${id}/delete`, { method: "PATCH" });
