<template>
  ...
  >
    ...
    <!-- Actual Content -->
    <div v-else>
      <h2 class="text-lg font-bold mb-2">Agent Registration Completion Rate</h2>

      <!-- Info -->
      <p class="text-sm text-white dark:text-gray-300 mb-2">
        {{ registeredCount }} of {{ invitedCount }} agents registered
      </p>
          ...
          <table class="w-full border-collapse border border-gray-300 dark:border-gray-600">
            ...
            <tbody>
              <tr>
                ...
                <td class="border p-2">If true, uses fake stats</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table class="w-full border-collapse border border-gray-300 dark:border-gray-600">
            ...
            <tbody>
              <tr>
                ...
                <td class="border p-2">
                  Fetches insurance agent stats. If <code>mock</code> is true, loads fake data instead of API call.
                </td>
              </tr>
            </tbody>
          </table>
...
async function fetchRegistrationData() {
  if (props.mock) {
    invitedCount.value = 10;
    registeredCount.value = 6;
    loading.value = false;
    return;
  }

  try {
    const data = await $fetch("/api/insurance-agent/registration-stats");
    invitedCount.value = data.invitedCount;
    registeredCount.value = data.registeredCount;
  } catch (err) {
    console.error("Failed to fetch insurance agent stats:", err);
  } finally {
    loading.value = false;
  }
}
