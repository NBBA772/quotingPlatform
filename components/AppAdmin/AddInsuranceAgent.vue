  ...
  <div class="p-4 my-4 shadow rounded-lg bg-white dark:bg-[#3a4934]">
    ...
    <form v-if="open" class="mt-4 space-y-4" @submit.prevent="submit">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label class="block text-gray-700 dark:text-gray-300 font-medium mb-1">Phone (optional)</label>
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400">
        The agent logs in with their email and this password.
      </p>
...
async function submit() {
  ...
  saving.value = true
  try {
    await $fetch('/api/insurance-agent/register', {
      method: 'POST',
      body: {
    ...
    message.value = `Agent login created for ${form.firstName} ${form.lastName} (${form.email}).`
    Object.assign(form, blank())
    passwordConfirm.value = ''
    emit('created')
  } catch (err: any) {
    error.value = err?.data?.statusMessage || err?.message || 'Failed to create agent login'
  } finally {
    saving.value = false
  }
}
