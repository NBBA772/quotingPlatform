<template>
  <div class="mx-auto px-6 py-6 bg-white dark:bg-[#3a4934] rounded-xl shadow-md space-y-6">
    <!-- <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Open Applications</h2> -->
      ...
      <tbody>
        <tr v-for="trail in auditTrails" :key="trail.id" class="hover:bg-gray-50 dark:hover:bg-[#2d3a2a]">
          ...
          <td class="p-2 dark:text-white">
            <a v-if="trail.paymentAuthorization?.pdfUrl" :href="trail.paymentAuthorization.pdfUrl" target="_blank" class="text-blue-500 hover:underline">
              View PDF
            </a>
            <span v-else>N/A</span>
          </td>
          <td class="p-2">
            <button class="text-red-500 hover:text-red-700" @click="confirmRemove(trail.id)" title="Remove Application">
...
interface AuditTrail {
  ...
  paymentAuthorization?: { pdfUrl?: string };
}
...
onMounted(async () => {
  try {
    const res = await $fetch("/api/admin/payment-auth-audit-trails/active"); // only active (deletedAt = null)
...
const confirmRemove = async (id: number) => {
  ...
  try {
    await $fetch(`/api/admin/payment-auth-audit-trails/${id}/delete`, { method: "PATCH" });
