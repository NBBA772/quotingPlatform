...
const { data, pending } = await useFetch<TopicData>(`/api/topic/${topicName}`, {
  key: route.fullPath,
});
...
function createIframe(vidId: string) {
  ...
  const htmlString = `
    <iframe class="mt-5 block object-cover object-center lg:rounded-lg w-full min-w-200 aspect-video mb-10 p-0" src="${url}" allowfullscreen allow="autoplay"></iframe>
  `;
      ...
      <section class="overflow-hidden text-gray-700 p-0">
        <div class="container px-0 py-2 mx-auto lg:pt-12 lg:px-32">
          <div
            ...
            @click="createIframe(activeVideo?.host_id)"
            ref="videoWrapper"
          >
            <div class="mt-5">
